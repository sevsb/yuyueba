...
class db_tempuser extends database_table {
    ...
    public function add($type, $openid, $uid, $nickname, $avatar, $create_time, $active_time, $last_login, $token, $status, $groups, $yuyue_session) {
        return $this->insert(array("type" => $type, "openid" => $openid, "uid" => 0, "nickname" => $nickname, "avatar" => $avatar, "create_time" => time(), "active_time" => $active_time, "last_login" => time(), "token" => $token, "status" => $status, "groups" => $groups, "yuyue_session" => $yuyue_session));
    }
